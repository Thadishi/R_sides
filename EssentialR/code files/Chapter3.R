...
## 2)## Make a histogram from the following data, and add a density estimate line to it. (use `scan()` to enter the data). Try changing the bandwith parameter for the density estimate (use the argument "adj" for density( ); 1 is the default, 2 means double the bandwidth). How does this change the density estimate?
26 30 54 25 70 52 51 26 67 18 21 29 17 12 18 35 30 36 36 21 24 18 10 43 28 15 26 27
# Note: If you are submitting this HW for class in a .txt file, you need't include the plot, just include a brief description of how changing the bandwidth parameter for `density()` alters the shape of the curve. Note that the bandwith argument here can be a string that matches on of several methods for bandwith calculation or a numeric value for the bandwidth.
